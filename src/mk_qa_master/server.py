    ...
    return [
        ...
        Tool(
            ...
            description=(
                ...
                "Plans live 30 minutes (cache TTL) in memory and are LRU-bounded "
                "at 50 outstanding.\n\n"
                "v0.9.3 — disk persistence: when QA_PROJECT_ROOT is set (or "
                "QA_PLAN_PERSIST=true), the plan is also dumped atomically to "
                "<QA_PROJECT_ROOT>/test-results/plans/<plan_id>.json. "
                "verify_plan transparently falls back to disk on in-memory "
                "misses, so plans survive process restarts and cache eviction. "
                "Expiry is still honored on disk reads — a TTL'd plan won't "
                "silently reload. Persistence is best-effort: filesystem errors "
        ...
        Tool(
            ...
            description=(
                ...
                "v0.9.3 — When persistence is enabled (see qa_plan), an "
                "in-memory cache miss transparently falls back to disk. The "
